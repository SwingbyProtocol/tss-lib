		...
		case save := <-endCh:
			...
			if atomic.LoadInt32(&ended) == int32(len(pIDs)) {
				...
				sk := schnorr.PrivateKey{
					PublicKey: *save.ECGDSAPub,
					D:         u,
				}

				// test pub key, should be on curve and match pkX, pkY
				assert.True(t, tss.EC().IsOnCurve(save.ECGDSAPub.X, save.ECGDSAPub.Y), "public key must be on curve")

				// public key tests
				assert.NotZero(t, u, "u should not be zero")
				ourPkX, ourPkY := tss.EC().ScalarBaseMult(u.Bytes())
				assert.Equal(t, save.ECGDSAPub.X, ourPkX, "pkX should match expected pk derived from u")
				assert.Equal(t, save.ECGDSAPub.Y, ourPkY, "pkY should match expected pk derived from u")
				...
				signature, err := sk.Sign(data)
				assert.NoError(t, err, "sign should not throw an error")
				ok := sk.PublicKey.Verify(data, signature.R, signature.S)
				assert.True(t, ok, "signature should be ok")
				t.Log("ECGDSA signing test done.")
