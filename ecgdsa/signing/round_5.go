...
func (round *round5) Start() *tss.Error {
	...
	minusH := modN.Sub(zero, round.temp.h)
	yminushx, yminusy := tss.EC().ScalarMult(round.key.ECGDSAPub.X, round.key.ECGDSAPub.Y, minusH.Bytes())
	// y^(-h)*R(-1)
	m1x, m1y := tss.EC().Add(yminushx, yminusy, round.temp.bigMinusQ.X(), round.temp.bigMinusQ.Y())
