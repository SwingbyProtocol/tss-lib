...
import (
	...
	"github.com/binance-chain/tss-lib/ecgdsa/keygen"
	"github.com/binance-chain/tss-lib/ecgdsa/signing"
	"github.com/binance-chain/tss-lib/tss"
)

// round 1 represents round 1 of the keygen part of the ECGDSA TSS spec
...
func (round *round1) Update() (bool, *tss.Error) {
	...
	for j, msg := range round.temp.dgRound1Messages {
		...
		// save the ecgdsa pub received from the old committee
		...
		if round.save.ECGDSAPub != nil &&
			!(candidate.Y.Cmp(round.save.ECGDSAPub.Y) == 0 && candidate.X.Cmp(round.save.ECGDSAPub.X) == 0) {
			// uh oh - anomaly!
			return false, round.WrapError(errors.New("eddsa pub key did not match what we received previously"), msg.GetFrom())
		}
		round.save.ECGDSAPub = candidate
	}
	return true, nil
}
