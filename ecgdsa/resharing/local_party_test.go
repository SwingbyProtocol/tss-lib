...
import (
	...
	. "github.com/binance-chain/tss-lib/ecgdsa/resharing"
	...
	for {
		...
		case signData := <-signEndCh:
			...
			if atomic.LoadInt32(&signEnded) == int32(len(signPIDs)) {
				...
				// BEGIN EDDSA verify
				pkX, pkY := signKeys[0].ECGDSAPub.X, signKeys[0].ECGDSAPub.Y
				pk := schnorr.PublicKey{
					X: pkX,
					Y: pkY,
				}

				newSig, err := schnorr.ParseSignature(signData.Signature)
				if err != nil {
					println("new sig error, ", err.Error())
				}
				ok := pk.Verify(big.NewInt(42).Bytes(),
					newSig.R, newSig.S)
